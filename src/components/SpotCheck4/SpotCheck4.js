...
class SpotCheck3 extends Component {
    ...
    this.state = {
      quotes: [
        ...
        },
        {
          id: '50xx11',
          text:
            "If you're going to walk in the rain, at least make sure it's raining",
          likes: 82,
        },
        {
          id: '50ggr2',
          text:
            "Don't stop climbing the mountain. Instead, keep climbing the mountain.",
          likes: 33,
        },
      ],
    }
  ...
  likeQuote = quoteId => {
    let newQoutes = [...this.state.quotes]
    newQoutes.find(qoute => qoute.id === quoteId).likes++
    this.setState({
      quotes: newQoutes,
    })
  }
    ...
    return (
      <div>
        {this.state.quotes.map(q => {
          return <Qoute key={q.id} quote={q} likeQuote={this.likeQuote} />
        })}
      </div>
    )
  }
